...
import pygame, labyrinthSprites, random
        ...
        for col in range(len(maze_arrangement[row])):
            if maze_arrangement[row][col] == 1: 
                if (row == 7 and col == 6) or (row == 6 and col == 8) or \
                   (row == 8 and col == 9):
                    stone = labyrinthSprites.Stone(random.randint(0,3), row*50, col*50, True)
                    maze_wall.add(stone)
                else:
                    stone = labyrinthSprites.Stone(random.randint(0,3), row*50, col*50, False)
                    maze_wall.add(stone)                    
    ...
    for gear_item in range(4):
        ...
        while not valid_place:
            ...
            # Ensuring the gear pieces only spawn on a valid pathway block
                ...
                powerup = labyrinthSprites.GearPieces(gear_item, row*50, col*50)
                gear_powerups.add(powerup)
                valid_place = True                

def game_instructions(screen):
    # DISPLAY ==================================================================
    background = pygame.image.load("./MiscImages/Introduction_Screen.jpg")
    screen.blit(background, (0, 0))    
    
    # ENTITIES ================================================================= 
    start_game_sound = pygame.mixer.Sound("./Sounds/metal_gong.wav")
...
def game(screen):
    ...
    # DISPLAY ==================================================================
    background = pygame.image.load("./MiscImages/background.jpg")
    ...
    pygame.mixer.music.load("./Sounds/eerie_music.mp3")
    pygame.mixer.music.set_volume(0.4)
    pygame.mixer.music.play(-1) 
    
    # The Minotaur's growl when his distance is "far" from the player
    far_minotaur_growl = pygame.mixer.Sound("./Sounds/monster_snarl.wav")
    far_minotaur_growl.set_volume(0.1)
    
    # The Minotaur's growl when his distance is "somewhat far" from the player
    med_minotaur_growl = pygame.mixer.Sound("./Sounds/monster_snarl.wav")
    med_minotaur_growl.set_volume(0.25) 
    
    # The Minotaur's growl when his distance is "close" to the player
    close_minotaur_growl = pygame.mixer.Sound("./Sounds/monster_snarl.wav")
    close_minotaur_growl.set_volume(0.5)       
    
    equip_gear_sound = pygame.mixer.Sound("./Sounds/sword_clash.wav")
    equip_gear_sound.set_volume(0.7)
    
    minotaur_death_sound = pygame.mixer.Sound("./Sounds/minotaur_death.wav")
    minotaur_death_sound.set_volume(1.0)
    
    minotaur_spawn_sound = pygame.mixer.Sound("./Sounds/metal_gong.wav")
    ...
    player_walking_sound = pygame.mixer.Sound("./Sounds/player_walking.wav")
    player_walking_sound.set_volume(0.5)
    
    # The sound played when the player collects all four pieces of gear
    all_gear_sound = pygame.mixer.Sound("./Sounds/player_full_gear.wav")
    all_gear_sound.set_volume(0.8)

    # Images
    
    game_over_message = pygame.image.load("./MiscImages/Game_Over_Screen.jpg")
    
    vision_space = pygame.image.load("./MiscImages/vision_limiter.png")

    player = labyrinthSprites.Player(screen, maze_arrangement)
    minotaur = labyrinthSprites.Minotaur(screen, maze_arrangement)
    visible_area = labyrinthSprites.VisionLimiter(screen, vision_space)
    ...
    health_tracker = labyrinthSprites.HealthKeeper(screen)
    gear_tracker = labyrinthSprites.GearTracker()
    countdown = labyrinthSprites.Countdown()
    
    allSprites = pygame.sprite.OrderedUpdates(maze_wall, gear_powerups, minotaur, player, visible_area, \
                                              gear_tracker, health_tracker, countdown)
    ...
    while keep_going:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_DOWN:
                    player.walk_down(player_walking_sound)
                    
                if event.key == pygame.K_UP:                 
                    player.walk_up(player_walking_sound)
                    
                if event.key == pygame.K_RIGHT:
                    player.walk_right(player_walking_sound)
                    
                if event.key == pygame.K_LEFT:                 
                    player.walk_left(player_walking_sound)           
        ...
        grace_period_countdown = 15 - (time_tracker/30)
...
def save_user_name(screen):
    # DISPLAY ==================================================================
    background = pygame.image.load("./MiscImages/Player_Name_Screen.jpg")
    screen.blit(background, (0, 0))    
    ...
    survivor_font = pygame.font.Font("./Fonts/PressStart2P.ttf", 18)
